...
import {
    ...
    FaCopy, FaTimes, FaChevronDown, FaChevronRight, FaSpinner, FaExclamationTriangle
} from 'react-icons/fa';
...
const ClinicalPharmacyTool = () => {
    ...
    const isPharmacistOrStudent =
        (isIndividual && (isPharmacist || isPharmacyStudent)) || isCompanyUser;
        ...
        const fetchLabTests = async () => {
            ...
            try {
                ...
                const grouped = (data || []).reduce((acc, test) => {
                    const catLabel = test.category || 'General';
                    if (!acc[catLabel]) {
                        acc[catLabel] = {
                            id: slugifyLabName(catLabel),
                            label: catLabel,
                            icon: FaVial,
                            tests: []
                        };
                    }
                    acc[catLabel].tests.push({
                        id: test.id,
                        name: test.name,
                        unit: test.unit || '',
                        range_male: test.range_male || '',
                        range_female: test.range_female || '',
                        description: test.description || '',
                        cdss_key: test.cdss_key || null,
                        key: getLabKey(test)
                    });
                    return acc;
                }, {});

                setLabSubCategories(Object.values(grouped));
            } catch (err) {
                console.error('Error fetching lab tests:', err);
                if (!cancelled) setLabsError('Failed to load lab tests. Please try again.');
            } finally {
        ...
        if (formData.weight && formData.height) {
            ...
            if (weight > 0 && height > 0) {
                ...
                if (formData.bsa !== calculatedBSA) {
                    setFormData((prev) => ({ ...prev, bsa: calculatedBSA }));
                }
            }
        }
    ...
    useEffect(() => {
        ...
        if (hasRequiredFields) {
            ...
            if (formData.labs?.egfr !== calculatedEGFR) {
                setFormData((prev) => ({
                    ...prev,
                    labs: { ...prev.labs, egfr: calculatedEGFR }
                }));
            }
        } else {
    ...
    const handleLabSubCategoryToggle = (subCategoryId) => {
        if (selectedLabSubCategories.includes(subCategoryId)) {
            setSelectedLabSubCategories(
                selectedLabSubCategories.filter((id) => id !== subCategoryId)
            );
        } else {
        ...
        try {
            ...
            autoTable(doc, {
                ...
                body: [
                    [
                        'Age & Gender:',
                        `${formData.age || 'N/A'} - ${formData.gender || 'N/A'}`
                    ],
                    [
                        'Vitals:',
                        `Weight: ${formData.weight || '-'} kg, BSA: ${
                            formData.bsa || '-'
                        } m², BP: ${formData.blood_pressure || '-'}`
                    ],
            ...
            if (cdssData && cdssData.length > 0) {
                const alertRows = cdssData.map((alert, index) => {
                    const isClient =
                        localStorage.getItem('userRole') === 'healthcare_client' ||
                        localStorage.getItem('pharmacare_userRole') === 'healthcare_client';
                    const finding = isClient
                        ? alert.client_message || alert.message
                        : alert.professional_message || alert.message;
                    const recommendation =
                        (isClient
                            ? alert.client_recommendation || alert.details
                            : alert.professional_recommendation || alert.details) ||
                ...
                doc.text(
                    'No drug-related problems detected based on current analysis.',
                    15,
                    currentY + 7
                );
                currentY += 15;
            ...
            if (formData.medications.length > 0) {
                ...
                autoTable(doc, {
                    startY: currentY + 5,
                    head: [['#', 'Drug Name', 'Dose']],
                    body: formData.medications.map((m, i) => [
                        i + 1,
                        m.drug_name,
                        m.dose
                    ]),
            ...
            doc.save(
                `Comprehensive_Report_${new Date().toISOString().split('T')[0]}.pdf`
            );
        } catch (error) {
            ...
            <div className="min-h-screen bg-gray-50 p-4 md:p-6 animate-in fade-in duration-500">
                <div className="max-w-6xl mx-auto">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        >
                            <FaNotesMedical /> Download Report
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600">
                                Fill the data for medication review to run clinical analysis
                            </p>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div className="p-4 pt-0">
                                            ...
                                            {!labsLoading && !labsError && labSubCategories.length === 0 && (
                                                <div className="text-center py-6 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                                    No lab tests configured. Please add them in Lab Settings.
                                                </div>
                                            )}
                                                        ...
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                            {labSubCategories.map((subCat) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <span className="ml-auto text-[10px] text-gray-400 bg-white px-1.5 py-0.5 rounded-full border border-gray-200">
                                                                        {subCat.tests.length}
                                                                    </span>
                                                                </label>
                                                            ))}
                                                                        ...
                                                                        {labCategory.tests.map((test) => {
                                                                            ...
                                                                            const refRange =
                                                                                formData.gender === 'female'
                                                                                    ...
                                                                                    ? test.range_male
                                                                                    : test.range_male || test.range_female;
                        ...
                        <div className="space-y-6">
                            ...
                            {selectedCategories.includes('special_conditions') && (
                                <div className="bg-white rounded-xl shadow-sm border-l-4 border-orange-500 overflow-hidden">
                                    ...
                                    >
                                        <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                                            <FaExclamationCircle className="text-orange-500" /> Special
                                            Conditions
                                        </h3>
                                        <div className="text-gray-400">
                                            ...
                                            <div className="grid grid-cols-1 gap-3">
                                                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                                                    ...
                                                    <span className="font-medium text-gray-700">
                                                        Kidney Failure / Impaired Renal Function
                                                    </span>
                                                </label>
                                                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                                                    <input
                                                    ...
                                                    />
                                                    <span className="font-medium text-gray-700">
                                                        Liver Failure / Hepatic Impairment
                                                    </span>
                                                </label>
                                                {formData.gender === 'female' && (
                                                    <>
                                            ...
                                            {!expandedSections.medications &&
                                                formData.medications.length > 0 && (
                                                    <span className="text-xs text-gray-400 mr-2">
                                                        ...
                                                        {formData.medications.length > 1 ? 's' : ''}
                                                    </span>
                                                )}
                                            <div className="text-gray-400">
                                                {expandedSections.medications ? (
