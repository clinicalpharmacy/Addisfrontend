...
const API_URL = import.meta.env.VITE_API_URL;
  ...
  const checkRegistrationStatus = () => {
    ...
    const userData = localStorage.getItem('registered_user');
    
    // Check if user just registered
    if (userData) {
      try {
        const parsedRegisteredUser = JSON.parse(userData);
        ...
        return; // Stop here if we have newly registered user
      } catch (e) {
        console.error('Error parsing registered user:', e);
      }
    }
    
    // If no registration found, redirect to registration
    if (!userData) {
...
const getPlans = () => {
  ...
  // Handle Chapa payment for newly registered users
  const handleChapaPaymentForNewUser = async () => {
    if (!selectedPlan) {
    ...
    console.log('Starting Chapa payment for new user:', plan);
    ...
    try {
      ...
      console.log('Chapa payment created successfully:', data);
  ...
  const handlePayment = async () => {
    ...
    // Always use Chapa for new users
    await handleChapaPaymentForNewUser();
  };
          ...
          {plans.map((plan) => (
            <div 
              ...
              className={`bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl ${
                selectedPlan === plan.id ? 'ring-2 ring-blue-500 ring-offset-2 transform scale-[1.02]' : ''
            ...
            <div className="mt-8 pt-8 border-t">
              ...
              <button
                ...
                className={`w-full py-4 px-6 rounded-lg font-semibold text-lg transition-colors ${
                  !selectedPlan || loading || isProcessingPayment
