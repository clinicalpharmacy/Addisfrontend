...
export const mapPatientToFacts = (patientData, medicationHistory = []) => {
    ...
    const extractLabValue = (source, key) => {
        ...
        const keyVariations = [
            ...
            `lab_${key}`,
            `${key}_lab`,
            `lab_${key.toLowerCase()}`,
            `${key.toLowerCase()}_lab`
        ];
...
const evaluateSingleCondition = (condition, facts, debug = false) => {
    ...
    if (patientValue === undefined) {
        ...
        // Check labs
        if (patientValue === undefined && facts.labs && facts.labs[fact]) {
            patientValue = facts.labs[fact];
        }
    ...
    if (debug) {
        console.log(`    Patient value:`, patientValue);
    }
