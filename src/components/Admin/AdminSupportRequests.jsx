...
import {
    ...
    FaUser, FaEnvelope, FaSync, FaUserShield
} from 'react-icons/fa';
import api from '../../utils/api';

/**
 * 🔐 AdminSupportRequests Component
 * Focuses exclusively on the Active Support Vault.
 * Troubleshooting access is granted instantly by users, so no approval queue is needed.
 */
export const AdminSupportRequests = () => {
    ...
    const handleReject = async (req) => {
        if (!window.confirm(`Revoke and close troubleshooting session with ${req.owner?.full_name || 'this user'}?`)) return;
        try {
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className="bg-white border border-gray-100 p-4 rounded-2xl text-gray-400 hover:text-blue-600 transition-all hover:shadow-lg"
                            title="Refresh Data"
                        >
