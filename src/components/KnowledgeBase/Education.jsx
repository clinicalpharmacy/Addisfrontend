...
import { FaGraduationCap, FaBookOpen, FaClipboardList, FaSearch, FaSpinner } from 'react-icons/fa';
...
const Education = () => {
    ...
    if (selectedExam) {
        return <ExamModule examId={selectedExam.id} examTitle={selectedExam.title} examSubject={selectedExam.subject} onBack={() => setSelectedExam(null)} />;
    }
            ...
            <div className="p-6 md:p-8 relative z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 mt-2 font-medium ml-1">
                            Test your knowledge and review clinical guidelines
                        </p>
                    </div>
                </div>

                {/* Tabs */}
                <div className="flex bg-gray-100/80 p-1.5 rounded-xl mb-8 max-w-md">
                    <button
                    ...
                    >
                        <FaClipboardList className={activeTab === 'exam' ? 'text-purple-500' : 'opacity-60'} />
                        Exams
                    </button>
                    <button
                    ...
                    >
                        <FaBookOpen className={activeTab === 'guideline' ? 'text-indigo-500' : 'opacity-60'} />
                        Guidelines
                    </button>
                </div>
                                        ...
                                        >
                                            ...
                                            <div>
                                                <h4 className="font-bold text-gray-800 group-hover:text-purple-700 transition-colors line-clamp-1">
                                                    {exam.title || `${exam.subject} Exam`}
                                                </h4>
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {exam.subject} • {exam.question_count || 0} Questions
                                                </p>
                                            </div>
                                        </button>
                                    ))}
                                ...
                                {filteredExams.length === 0 && (
                                    <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-100">
                                        <p className="text-gray-500 font-medium">
                                            {exams.length === 0 ? 'No exams available at the moment.' : `No exams found matching "${searchTerm}"`}
                                        </p>
                                    </div>
                                )}
                            </>
                ...
                {activeTab === 'guideline' && (
                    <div className="animate-fadeIn bg-indigo-50 border border-indigo-100 rounded-xl p-8 text-center">
                        ...
                        <h3 className="text-xl font-bold text-indigo-900 mb-2">Clinical Guidelines</h3>
                        <p className="text-indigo-700 max-w-md mx-auto">
                            The guidelines section is currently under development. Please check back later for comprehensive medical guidelines across all specialties.
                        </p>
                    </div>
                )}
            </div>
